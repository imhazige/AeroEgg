...
namespace kazgame.character {
	...
	public class CharacterController2D : MonoBase
	{
		...
		public class CharacterCollisionState2D
		{
			...
			{
				return string.Format( "[CharacterCollisionState2D] r: {0}, l: {1}, a: {2}, b: {3}, movingDownSlope: {4}, angle: {5}, wasGroundedLastFrame: {6}, becameGroundedThisFrame: {7}, movingUpSlope: {8}",
					right, left, above, below, movingDownSlope, slopeAngle, wasGroundedLastFrame, becameGroundedThisFrame,movingUpSlope );
			}
		}
		...
		public void move( Vector3 deltaMovement )
		{
			...
			if (Math.Abs (deltaMovement.x) > Mathf.Epsilon)
				moveHorizontally (ref deltaMovement);

			// next, check movement in the vertical dir
			if (Math.Abs (deltaMovement.y) > Mathf.Epsilon)
		...
		private void handleVerticalSlope( ref Vector3 deltaMovement )
		{
			...
			if( _raycastHit )
			{
				...
				var isMovingDownSlope = Math.Abs (Mathf.Sign (_raycastHit.normal.x) - Mathf.Sign (deltaMovement.x)) < Mathf.Epsilon;
